    ...
    with rasterio.open(
        ...
        for i in range(0, height, chunk_size):
            for j in range(0, width, chunk_size):
                ...
                # 写入
                dst.write(chunk_data.values.astype('uint8'), 1, window=window)
...
def main():
    ...
    # ——在 encode_pair() 末尾加上——
    y_dim = encoded.rio.y_dim          # 通常 'y' 或 'latitude'
    x_dim = encoded.rio.x_dim          # 通常 'x' 或 'longitude'
    if encoded.ndim > 2:
        encoded = encoded.squeeze()
    if encoded.dims != (y_dim, x_dim):
        encoded = encoded.transpose(y_dim, x_dim)

    # 用 2048 级别的块（和你的切窗 8192 协同）
    encoded = encoded.chunk({y_dim: 2048, x_dim: 2048})
    ...
    write_cog(
        encoded.astype("uint8"),
        encoded_path,
        overwrite=True,
        blocksize=512,                 # 与原手写循环保持一致
        compress="deflate",
        zlevel=9,
        nodata=255,
        overview_levels=(2, 4, 8, 16, 32, 64),
        overview_resampling="nearest",
        use_windowed_writes=True,      # 关键：逐块写，避免一次性 compute
    )
    ...
    # # 保证 2D + (y,x) 顺序 + 合理 chunk
    # y_dim = encoded.rio.y_dim
    # x_dim = encoded.rio.x_dim
    # if encoded.ndim > 2:
    #     encoded = encoded.squeeze()
    # if encoded.dims != (y_dim, x_dim):
    #     encoded = encoded.transpose(y_dim, x_dim)
